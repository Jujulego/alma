import { RequestOptions, useApi } from '@jujulego/alma-api';
...
// Hook
export function useGqlHttp<D, V extends GqlVars = GqlVars>(url: string, doc: GqlDoc<V>, options?: RequestOptions<'json'>): RequestSender<D, V> {
  // Request
  const _doc = useDeepMemo(doc);
  const request = useMemo(() => buildRequest(_doc), [_doc]);

  // Api
  const send = useApi<GqlResponse<D>, GqlRequest<V>>('post', url, options);
